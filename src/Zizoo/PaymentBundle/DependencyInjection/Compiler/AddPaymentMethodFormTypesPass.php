...
namespace Zizoo\PaymentBundle\DependencyInjection\Compiler;
...
class AddPaymentMethodFormTypesPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->hasDefinition('zizoo_payment.payment_method_type')) {
            return;
        }
        
        $paymentMethodFormTypes = array();
        foreach ($container->findTaggedServiceIds('zizoo_payment.method_form_type') as $id => $attributes) {
        ...
        $container->getDefinition('zizoo_payment.payment_method_type')
